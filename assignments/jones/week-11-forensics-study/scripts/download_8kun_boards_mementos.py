...
with open("8ch.net-timemap.json") as f:
    jdata = json.load(f)
...
for entry in jdata["mementos"]["list"]:
    ...
    output_filename = 'boards-mementos/' + entry["datetime"] + '.html'
    headers_output_filename = 'boards-mementos/' + entry["datetime"] + '-headers.json'
